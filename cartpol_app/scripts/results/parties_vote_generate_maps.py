...
def merge_data_with_shapefile(gdf, df_data, level):
    ...
    if level == 'municipio':
        ...
        # Para nível estado, fazer merge por código do município ou nome
        # Tentar primeiro por código (mais confiável)
        possible_name_cols = ['NM_MUN', 'NOME_MUNICIPIO', 'municipio']

        # Fazer merge por nome normalizado
        name_col_shape = None
        ...
        if name_col_shape is None:
            print(
                f"  - Colunas disponíveis no shapefile: "
                f"{list(gdf.columns)}"
            )
            raise ValueError(
                "Não foi possível encontrar coluna de município "
                "no shapefile"
            )

        print(
            f"  - Usando coluna de nome '{name_col_shape}' "
            f"do shapefile"
        )
        print("  - Fazendo merge por nome normalizado")

        # Normalizar nomes para fazer match (como no nível município)
        gdf['municipio_normalized'] = gdf[name_col_shape].apply(
            normalize_name_for_match
        )
        df_data['municipio_normalized'] = df_data['municipio'].apply(
            normalize_name_for_match
        )
        print(gdf['municipio_normalized'].head(5))
        print(df_data['municipio_normalized'].head(5))

        # Log para debug
        print(
            f"  - Valores únicos no shapefile: "
            f"{gdf['municipio_normalized'].nunique()}"
        )
        print(
            f"  - Valores únicos no CSV: "
            f"{df_data['municipio_normalized'].nunique()}"
        )
        print(f"  - Registros no CSV: {len(df_data)}")

        # Fazer merge
        gdf_merged = gdf.merge(
            df_data,
            left_on='municipio_normalized',
            right_on='municipio_normalized',
            how='left'
        )

        # Remover coluna auxiliar
        gdf_merged = gdf_merged.drop(columns=['municipio_normalized'])

    else:
        raise ValueError(f"Nível inválido: {level}")

    print(f"  - Features após merge: {len(gdf_merged)}")
    # Verificar se há dados (pode ser sigla_partido ou nome_partido)
    check_col = 'sigla_partido' if 'sigla_partido' in gdf_merged.columns else 'nome_partido'
    if check_col in gdf_merged.columns:
        print(
            f"  - Features com dados: "
            f"{gdf_merged[gdf_merged[check_col].notna()].shape[0]}"
        )
...
def generate_map(
    gdf_merged, indicator_column, party_name, indicator_name,
    ...
    Args:
        ...
        party_name: Nome ou sigla do partido
...
def generate_maps(
    csv_file, party_sigla, level, municipio_id, uf, year,
    ...
    # Filtrar por partido
    df_party = df_data[df_data['sigla_partido'] == party_sigla]
    if df_party.empty:
        available = df_data['sigla_partido'].unique()[:10]
        raise ValueError(
            f"Partido '{party_sigla}' não encontrado nos dados. "
            f"Partidos disponíveis: {available}"
        )
    print(f"  - Registros do partido: {len(df_party)}")

    # Obter nome do partido para exibição
    party_name = df_party['nome_partido'].iloc[0] if 'nome_partido' in df_party.columns else party_sigla
    print(f"  - Partido: {party_name} ({party_sigla})")
    ...
    # Fazer merge
    gdf_merged = merge_data_with_shapefile(gdf, df_party, level)

    # Normalizar sigla do partido para nome de arquivo
    party_sigla_safe = re.sub(r'[^a-zA-Z0-9]', '_', party_sigla)

    # Verificar se as colunas LQ, HC e CI existem
    has_lq = 'LQ' in gdf_merged.columns
    has_hc = 'HC' in gdf_merged.columns
    has_ci = 'CI' in gdf_merged.columns
    ...
    rcan_uesp_path = os.path.join(
        output_dir,
        f'mapa_RCAN_UESP_{party_sigla_safe}_{year}.png'
    )
    generate_map(
        gdf_merged, 'RCAN_UESP(%)', party_name,
    ...
    ruesp_can_path = os.path.join(
        output_dir,
        f'mapa_RUESP_CAN_{party_sigla_safe}_{year}.png'
    )
    generate_map(
        gdf_merged, 'RUESP_CAN(%)', party_name,
    ...
    if has_lq:
        lq_path = os.path.join(
            output_dir,
            f'mapa_LQ_{party_sigla_safe}_{year}.png'
        )
        generate_map(
            gdf_merged, 'LQ', party_name,
    ...
    if has_hc:
        hc_path = os.path.join(
            output_dir,
            f'mapa_HC_{party_sigla_safe}_{year}.png'
        )
        generate_map(
            gdf_merged, 'HC', party_name,
            'HC (Horizontal Cluster)', year, hc_path, level,
            cmap='RdBu', label_suffix=''
        )
        maps_generated.append(hc_path)

    # Mapa CI (se disponível)
    if has_ci:
        ci_path = os.path.join(
            output_dir,
            f'mapa_CI_{party_sigla_safe}_{year}.png'
        )
        generate_map(
            gdf_merged, 'CI', party_name,
            'CI (Carvalho Index)', year, ci_path, level,
            cmap='YlOrRd', label_suffix=''
        )
        maps_generated.append(ci_path)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Gera mapas temáticos com indicadores de votação por partido'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--party-sigla', type=str, required=True,
        help='Sigla do partido para gerar os mapas (ex: PT, PSDB)'
    )
    parser.add_argument(
    ...
    try:
        generate_maps(
            args.csv_file, args.party_sigla, args.level,
...
'''
Examples:
python cartpol_app/scripts/results/parties_vote_generate_maps.py \
    --csv-file cartpol_app/scripts/results/script_results/resultado_votacao_partidos_estado_2018.csv \
    --party-sigla "PSL" --level estado --uf RJ --year 2018

python cartpol_app/scripts/results/parties_vote_generate_maps.py \
    --csv-file cartpol_app/scripts/results/script_results/resultado_votacao_partidos_municipio_2022.csv \
    --party-sigla "PT" --level municipio --municipio-id 60011 --uf RJ --year 2022
'''
